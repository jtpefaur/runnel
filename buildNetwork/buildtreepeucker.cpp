...
BuildTreePeucker::BuildTreePeucker() {
    shader = 0;
    orderThreshold = 1;
    QObject::connect(&conf, SIGNAL(changeOrderThreshold()), this, SLOT(changeOrderThreshold()));
}
...
std::vector<arbol*> BuildTreePeucker::run(Terrain* ter) {
    ...
    types.clear();
    shader->fillPositionBuffer(arbolitos, types, this->orderThreshold);
    return arbolitos;
}
...
void BuildTreePeucker::changeOrderThreshold() {
    this->orderThreshold = conf.getOrderThreshold();
    this->run(t);
    emit reload();
}
